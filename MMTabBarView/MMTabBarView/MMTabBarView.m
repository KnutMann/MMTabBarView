...
//        [self setHidden:YES];
	...
	NSView* const partnerView = _partnerView;
	if (partnerView != nil) {
            // resize self and view
		NSRect resizeRect;
		if (self.orientation == MMTabBarHorizontalOrientation) {
			resizeRect = NSMakeRect(partnerView.frame.origin.x, partnerTargetOrigin, partnerView.frame.size.width, partnerTargetSize);
		} else {
			resizeRect = NSMakeRect(partnerTargetOrigin, partnerView.frame.origin.y, partnerTargetSize, partnerView.frame.size.height);
		}
